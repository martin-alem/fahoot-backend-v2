...
export class SharedService {
  ...
  public async signToken<T extends object>(entity: T, ttl: number = DEFAULT_SIGN_TOKEN_TTL): Promise<Result<string | null>> {
    ...
    if (!audience || !issuer || !secret) {
      return new Result<string | null>(false, null, 'JWT configuration is missing', HttpStatus.INTERNAL_SERVER_ERROR);
    }
    ...
    try {
      ...
      return new Result<string | null>(false, null, `Token signing failed: ${error.message}`, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  ...
  async validateToken<T extends object>(token: string): Promise<Result<T | null>> {
    ...
    if (!audience || !issuer || !secret) {
      return new Result<T | null>(false, null, 'JWT configuration is missing', HttpStatus.INTERNAL_SERVER_ERROR);
    }

    try {
      const decodedToken = this.jwtService.verify<T>(token, { audience, issuer, secret });
  ...
  public async hash(data: string): Promise<Result<string | null>> {
    try{
      ...
      return new Result<string | null>(false, null, "unable to hash data", HttpStatus.INTERNAL_SERVER_ERROR)
    }
  }
  ...
  public async compare(incomingData: string, hashedData: string): Promise<Result<boolean | null>> {
    try{
      ...
      return new Result<boolean | null>(false, false, "unable to compare data", HttpStatus.INTERNAL_SERVER_ERROR)
    }
  }

  public async generateToken<T extends object>(entity: T, tokenTTL: number = DEFAULT_SIGN_TOKEN_TTL): Promise<Result<string | null>> {
    try {
      ...
      return new Result<string | null>(false, null, "error generating token", HttpStatus.INTERNAL_SERVER_ERROR)
    }
  }
}
