...
export class UserService {
  ...
  constructor(@InjectRepository(User) usersRepository: Repository<User>, sharedService: SharedService, emailNotificationService: EmailNotificationService, configService: ConfigService) {
  ...
  async updateEmail(updateEmailDto: UpdateEmailDto, id: number): Promise<Result<boolean | null>>{
    ...
    try {
      ...
      if (user.authenticationType !== AuthenticationType.Email){
        return new Result<boolean | null>(false, null, "you have a google account and account change your email", HttpStatus.BAD_REQUEST)
      }
      ...
      const token = await this.sharedService.generateToken<JWTUser>({id: user.id, email: updateEmailDto.email, verified: user.verified})
  ...
  async verifyEmail(token: string): Promise<Result<boolean | null>>{
    try{
      ...
      await this.usersRepository.update(result.getData().email, {verified: true})
  ...
  async passwordResetRequest(passwordResetDto: PasswordResetDto): Promise<Result<boolean | null>>{
    try {
      ...
      const request: EmailNotificationRequest = {
        ...
        templateId: this.configService.get<string>("PASSWORD_RESET_TEMPLATE_ID"),
        recipients: [user.email],
        templateData: [{
          email: user.email,
          data: {
  ...
  async switchAccountType(switchAccountDto: SwitchAccountDto, id: number): Promise<Result<boolean | null>>{
    try {
      ...
      if(newAccountType === AuthenticationType.Email){
        ...
        const token = await this.sharedService.generateToken<JWTUser>({id: user.id, email: switchAccountDto.email, verified: user.verified})
